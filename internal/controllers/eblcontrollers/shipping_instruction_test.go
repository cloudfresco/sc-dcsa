...
func TestCreateShippingInstruction(t *testing.T) {
	...
	req, err := http.NewRequest("POST", "https://localhost:9061/v0.1/shipping-instructions", bytes.NewBuffer(data))
	if err != nil {
		t.Error(err)
		return
	}
...
func TestUpdateShippingInstruction(t *testing.T) {
	...
	req, err := http.NewRequest("PUT", "https://localhost:9061/v0.1/shipping-instructions/SI_REF_4", bytes.NewBuffer(data))
	if err != nil {
		t.Error(err)
		return
	}
