...
import (
	...
	ovsservices "github.com/cloudfresco/sc-dcsa/internal/services/ovsservices"
	partyservices "github.com/cloudfresco/sc-dcsa/internal/services/partyservices"

	ovsworkers "github.com/cloudfresco/sc-dcsa/internal/workers/ovsworkers"
...
func TestMain(m *testing.M) {
	...
	logOvs = config.SetUpLogging(logOpt.OvsPath)
	...
	pwd, _ := os.Getwd()
	go ovsservices.StartOvsServer(logOvs, true, pwd, dbOpt, redisOpt, mailerOpt, grpcServerOpt, jwtOpt, oauthOpt, userOpt, uptraceOpt, dbService, redisService, mailerService)
	...
	go ovsworkers.StartOvsWorker(log, true, pwd, grpcServerOpt, configFilePath)
