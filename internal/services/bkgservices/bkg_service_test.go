...
func TestBkgService_CreateBooking(t *testing.T) {
	fmt.Println("TestBkgService_CreateBooking started")
	err := test.LoadSQL(logUser, dbService)
	if err != nil {
		t.Error(err)
		return
	...
	for _, tt := range tests {
		...
		fmt.Println("TestBkgService_CreateBooking bookingResponse", bookingResponse)
		fmt.Println("TestBkgService_CreateBooking err", err)
		if (err != nil) != tt.wantErr {
